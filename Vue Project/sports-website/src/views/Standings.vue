<template>
  <div class="standings">
        <div id="main" class="mx-5 my-3">
           <h2 class="title">National Football Conference</h2>
           <table class="table table-striped">
              <tbody id="NFCNorthRows">
                 <StandingsHeading @sort-click="sort" division="NFC North"/>
              ...
              <tbody id="NFCEastRows">
                 <StandingsHeading @sort-click="sort" division="NFC East"/>
              ...
              <tbody id="NFCSouthRows">
                 <StandingsHeading @sort-click="sort" division="NFC South"/>
              ...
              <tbody id="NFCWestRows">
                 <StandingsHeading @sort-click="sort" division="NFC West"/>
         ...
         <h2 class="title">American Football Conference</h2>
            <table class="table table-striped">
               <tbody id="AFCNorthRows">
                 <StandingsHeading @sort-click="sort" division="AFC North"/>
              ...
              <tbody id="AFCEastRows">
                 <StandingsHeading @sort-click="sort" division="AFC East"/>
              ...
              <tbody id="AFCSouthRows">
                 <StandingsHeading @sort-click="sort" division="AFC South"/>
              ...
              <tbody id="AFCWestRows">
                 <StandingsHeading @sort-click="sort" division="AFC West"/>
...
<script>
   ...
   export default{
      ...
      methods:{
         ...
         initTeamStats(){
            for(let i=0;i<this.teams.length;i++){
               ...
               let teamGames = this.scores.filter(game=>game.AwayTeam===team.abbrev||game.HomeTeam===team.abbrev)
               ...
               for(let j=0; j<teamGames.length;j++){
                     ...
                     if(game.AwayTeam===team.abbrev){
                        if(game.AwayScore>game.HomeScore){
                           ...
                           if(type===0)
                                 this.teams[i].ConferenceWins++;
                           else
                                 this.teams[i].NonconferenceWins++;
                           if(type===-1)
                                 this.teams[i].DivisionWins++;
                           if(j<5)
                                 this.teams[i].Last5Wins++;
                           if(j===0)
                                 this.teams[i]["StreakType"]="W"
                           ...
                           if(type===0)
                                 this.teams[i].ConferenceLosses++;
                           else
                                 this.teams[i].NonconferenceLosses++;
                           if(type===-1)
                                 this.teams[i].DivisionLosses++;
                           if(j<5)
                                 this.teams[i].Last5Losses++;
                           if(j===0)
                                 this.teams[i]["StreakType"]="L"
                           ...
                           if(type===0)
                                 this.teams[i].ConferenceTies++;
                           else
                                 this.teams[i].NonconferenceTies++;
                           if(type===-1)
                                 this.teams[i].DivisionTies++;
                           if(j<5)
                                 this.teams[i].Last5Ties++;
                           if(j===0)
                                 this.teams[i]["StreakType"]="T"
         ...
         sort(field){
               ...
               if(this.sortDirection==='DSC'){
                  if(field==='team')
                     ...
                     this.teams.sort((a,b)=>(a.Wins/(a.Wins+a.Losses+a.Ties)-b.Wins/(b.Wins+b.Losses+b.Ties)));
                  else if(field==="PF")
                     this.teams.sort((a,b)=>(a.PF-b.PF));
                  else if(field==='PA')
                     this.teams.sort((a,b)=>(a.PA-b.PA));
                  else if(field==='net')
                     ...
                     this.teams.sort((a,b)=>((a.DivisionWins/(a.DivisionWins+a.DivisionLosses+a.DivisionTies))-(b.DivisionWins/(b.DivisionWins+b.DivisionLosses+b.DivisionTies))));
                  else if(field==='conf')
                     this.teams.sort((a,b)=>(a.ConferenceWins-b.ConferenceWins));
                  else if(field==='confPCT')
                     this.teams.sort((a,b)=>((a.ConferenceWins/(a.ConferenceWins+a.ConferenceLosses+a.ConferenceTies))-(b.ConferenceWins/(b.ConferenceWins+b.ConferenceLosses+b.ConferenceTies))));
                  ...
                  if(field==='team')
                     ...
                     this.teams.sort((b,a)=>a.Wins-b.Wins);
                  else if(field==='losses')
                     this.teams.sort((b,a)=>(a.Losses-b.Losses));
                  else if(field==='PCT')
                     this.teams.sort((b,a)=>(a.Wins/(a.Wins+a.Losses+a.Ties)-b.Wins/(b.Wins+b.Losses+b.Ties)));
                  else if(field==="PF")
                     this.teams.sort((b,a)=>(a.PF-b.PF));
                  else if(field==='PA')
                     this.teams.sort((b,a)=>(a.PA-b.PA));
                  else if(field==='net')
                     ...
                     this.teams.sort((b,a)=>((a.DivisionWins/(a.DivisionWins+a.DivisionLosses+a.DivisionTies))-(b.DivisionWins/(b.DivisionWins+b.DivisionLosses+b.DivisionTies))));
                  else if(field==='conf')
                     this.teams.sort((b,a)=>(a.ConferenceWins-b.ConferenceWins));
                  else if(field==='confPCT')
                     this.teams.sort((b,a)=>((a.ConferenceWins/(a.ConferenceWins+a.ConferenceLosses+a.ConferenceTies))-(b.ConferenceWins/(b.ConferenceWins+b.ConferenceLosses+b.ConferenceTies))));
                  else if(field==='nonconf')
      ...
      async created(){
         this.fetchScores().then(data=>{this.scores = data; this.scores = this.scores.filter(game=>game.Status!="Postponed"); this.initTeamStats(); this.filterTeams(); this.loaded=true})
      }
   }
</script>
